...
function initProblem(base,Y, id,sharelink,data, container) {
  ...
  var onResult = function(x) { 
    returned++
    if (returned == sent) {      
      if (x && String(x).toLowerCase() == "true") {        
        var shareLink = successDiv.one('#sharesolution')
        var shareButton = Y.Node.create('<span></span>')
        shareLink.get('childNodes').remove()
        shareLink.append(shareButton)
        addthis.button(shareButton._node, {
          templates: {
